...
####################################################################
    SparseBoundary:=function(n,s)
        ...
        if n=1
            ...
            g:=gamma(s[1]);
            b2:=[o[2]*SignInt(s[1])*b[2],g*s[2]];
            AddDictionary(SparseRecord,[n,s],[b1,b2]);
        else
            b3:=ShallowCopy(b);
            Apply(b3, x->[SignInt(s[1])*x,One(G)]);
            jigsaw:=[];
            Add(jigsaw, b3[1]);
            ...
            for i in [1..Length(jigsaw)]
                do
                jigsaw[i]:=[jigsaw[i][1]*o[i],jigsaw[i][2]*s[2]];
            od;
...
G:=FundamentalGroupOfRegularCWComplex(K, "no");;
